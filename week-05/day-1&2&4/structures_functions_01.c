...
void List() {
	...
	while (strlen(books[counter].Title) != 0) {
		printf("\t%d. %s - %s %s - %d\n", counter + 1, books[counter].Title, books[counter].Author.Firstname, books[counter].Author.Lastname, books[counter].Year);
		counter++;
	}
...
void Create(char param[]) {
	...
	char *p_auth_fn = strtok(p_auth, " ");
	char *p_auth_ln = strtok(NULL, " ");
	...
	strcpy(books[index].Author.Firstname, p_auth_fn);
	strcpy(books[index].Author.Lastname, p_auth_ln);
...
void ModifyAuth(char param[]) {
	...
	strcpy(books[atoi(p_id) - 1].Author.Firstname, p_auth_fn);
	strcpy(books[atoi(p_id) - 1].Author.Lastname, p_auth_ln);
