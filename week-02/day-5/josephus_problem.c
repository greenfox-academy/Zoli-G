...
//Let J(n) represent the indexed place for surviving the "game"; J(n) is a function of n.
//If we do this algo in graphically, we get the following result:
//J(n)  1 	1 	3 	1 	3 	5 	7 	1 	3 	5 	7 	9 	11 	13 	15 	 1   3   5   7   9  etc.

//The value of J(n) resets itself when n = 2^m. Meaning in these cases the safe position become 1 again.
//Any other position we can write n = 2^m + l. The safe positions are odd numbers starting from 1.
//If n = 2^m + l    ->  J(n) = 2 * l + 1 (odd).

//Task is to calculate J(n) from n:
